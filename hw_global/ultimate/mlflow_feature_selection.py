...
if args.shap_calculation:
    ...
    # Exclude 'local_hour' from X_selected and feature names
    if 'local_hour' in X_selected.columns:
        X_selected_filtered = X_selected.drop(columns=['local_hour'])
        feature_names_filtered = X_selected_filtered.columns.tolist()
        print("Excluded 'local_hour' from features.")
    else:
        X_selected_filtered = X_selected.copy()
        feature_names_filtered = X_selected_filtered.columns.tolist()
        print("'local_hour' not found in features.")
    ...
    X_selected_filtered.to_feather(X_path)
    mlflow.log_artifact(X_path)
    print(f"Saved filtered X data to {X_path}")

    # Create SHAP summary plot without 'local_hour'
    print("Creating SHAP summary plot without 'local_hour' feature...")
    shap.summary_plot(
        shap_values,
        X_selected_filtered,
    ...
    shap_feature_importance = get_shap_feature_importance(shap_values, feature_names_filtered, df_daily_vars)
    shap_feature_importance['Feature Group'] = shap_feature_importance['Feature'].apply(
        lambda x: get_feature_group(x, df_daily_vars)
    )

    # Group the SHAP values by feature group
    print("Grouping SHAP values by feature groups...")
    feature_to_group = dict(zip(feature_names_filtered, shap_feature_importance['Feature Group']))
    unique_groups = shap_feature_importance['Feature Group'].unique()

    # Create a mapping from group to feature indices
    group_to_indices = {}
    for idx, feature_name in enumerate(feature_names_filtered):
        group = feature_to_group[feature_name]
        group_to_indices.setdefault(group, []).append(idx)

    # Sum SHAP values within each group
    grouped_shap_values = np.zeros((shap_values.shape[0], len(unique_groups)))
    for i, group in enumerate(unique_groups):
        indices = group_to_indices[group]
        grouped_shap_values[:, i] = shap_values[:, indices].sum(axis=1)

    # Create SHAP summary plot by feature groups
    print("Creating SHAP summary plot by feature groups...")
    shap.summary_plot(
        grouped_shap_values,
    ...
    # Calculate feature importance by group
    shap_feature_importance_by_group = pd.DataFrame({
        'Feature Group': unique_groups,
        'Importance': np.abs(grouped_shap_values).mean(axis=0)
    })
    ...
    # Save and log the grouped feature importance data
    shap_feature_importance_by_group_path = os.path.join(
        figure_dir, f'{args.time_period}_shap_feature_importance_by_group.csv'
    )
    ...
    # Plot percentage contribution of each feature group
    print("Creating percentage contribution plot by feature groups...")
    plt.figure(figsize=(12, 0.5 * len(shap_feature_importance_by_group)))
    plt.barh(
        shap_feature_importance_by_group['Feature Group'],
        shap_feature_importance_by_group['Percentage'],
        align='center', color='#ff0051'
    )
    plt.title(f'Feature Group Importance (%) for {args.time_period.capitalize()}time')
    plt.xlabel('Percentage Contribution')
    plt.gca().invert_yaxis()

    for i, percentage in enumerate(shap_feature_importance_by_group['Percentage']):
        plt.text(percentage, i, f' {percentage:.1f}%', va='center')

    plt.tight_layout()
    mlflow.log_figure(plt.gcf(), f'{args.time_period}_percentage_contribution_by_group_plot.png')
    plt.clf()
...
if args.shap_calculation:
    ...
    feature_names = full_pool.get_feature_names()
    feature_names_path = os.path.join(figure_dir, 'feature_names.txt')
    with open(feature_names_path, 'w') as f:
    ...
    # Group the SHAP values by feature group
    print("Grouping SHAP values by feature groups...")
    feature_to_group = dict(zip(feature_names, shap_feature_importance['Feature Group']))
    unique_groups = shap_feature_importance['Feature Group'].unique()
